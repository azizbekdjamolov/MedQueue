...
function ClinicCard({ clinic }) {
  const t = useT()
  return (
    <div className="glass flex h-full flex-col gap-3 rounded-2xl p-5 transition-all duration-300 hover:border-border-strong hover:shadow-[var(--glow-soft)]">
      <div className="flex items-start justify-between gap-3">
        <h3 className="font-display text-base font-semibold leading-tight text-fg">
          {clinic.name}
        </h3>
        <Badge tone="neon">{clinic.typeName}</Badge>
      </div>
      ...
      <div className="mt-auto flex items-center justify-between gap-2 border-t border-border pt-3">
        <div className="flex flex-col">
          <span className="text-[11px] text-faint">{fmt(t, 'search.queueNow', { n: clinic.queueNow })}</span>
...
function DoctorCard({ doctor, onTakeQueue }) {
  const t = useT()
    ...
    <div className="glass flex h-full flex-col gap-3 rounded-2xl p-5 transition-all duration-300 hover:border-border-strong hover:shadow-[var(--glow-soft)]">
      ...
      <div className="mt-auto flex items-center justify-between gap-2 border-t border-border pt-3">
        ...
        >
          ...
          {joining ? t('search.joining') : t('search.takeQueue')}
        </button>
      </div>
    </div>
  )
...
export default function SearchPage({ navigate }) {
  ...
  const loadClinics = useCallback(async () => {
    ...
    try {
      const data = await fetchClinics(
        { district, specialty, type, q: q.trim(), sort: tab === 'clinics' ? sort : '' },
        lang
      )
  ...
  }, [district, specialty, type, q, sort, tab, lang])
  ...
  }, [tab, loadClinics, loadDoctors])

  useEffect(() => {
    let cancelled = false
    Promise.all([
      fetchDistricts(lang),
      fetchSpecialties(lang),
      fetchClinicTypes(lang),
    ])
      .then(([d, s, c]) => {
        if (cancelled) return
  ...
  const clinicsLoading = clinics === null
  const doctorsLoading = doctors === null

  return (
    <div className="relative px-4 pb-20 pt-24 sm:px-8">
      <div className="mx-auto max-w-7xl">
        <SectionHeading eyebrow={t('search.eyebrow')} title={t('search.title')} subtitle={t('search.subtitle')} />
          ...
          {['clinics', 'doctors'].map((key) => (
            <button
              ...
              className={
                ...
                (tab === key
                  ? 'bg-gradient-to-r from-neon-600 to-electric-600 text-white shadow-[0_2px_12px_rgba(139,92,246,0.35)]'
                  : 'border border-border bg-card text-muted hover:bg-card-hover hover:text-fg')
              }
            >
              {t(key === 'clinics' ? 'search.tabClinics' : 'search.tabDoctors')}
            </button>
          ))}
        </div>
